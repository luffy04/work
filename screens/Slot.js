...
export default class Slot extends React.Component {
  ...
  renderGallery = () => {
    ...
    const slotImages = [slot.image, slot.image, slot.image, slot.image];
      ...
      >
        {slotImages.map((image, index) => (
          <TouchableWithoutFeedback
            ...
            onPress={() => navigation.navigate('Gallery', { images: slotImages, index })}>
            <Image
              resizeMode="cover"
              source={{ uri: image }}
          ...
          <Block flex style={styles.options}>
            ...
            <ScrollView vertical={true} showsVerticalScrollIndicator={false}>
              <Block style={{ paddingHorizontal: theme.SIZES.BASE, paddingTop: theme.SIZES.BASE * 2 }}>
                <Text size={28} style={{ paddingBottom: 24 }}>{slot.title}</Text>
